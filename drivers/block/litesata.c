...
static void litesata_devm_irq_off(void *lsirq)
{
	litex_write32((void __iomem *)lsirq + LITESATA_IRQ_ENA, 0);
}
			     ...
			     struct litesata_dev *lbd)
{
	...
	ret = devm_add_action_or_reset(dev, litesata_devm_irq_off, lbd->lsirq);
	if (ret)
